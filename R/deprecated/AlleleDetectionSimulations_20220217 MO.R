...
(boxplot.B <- ggplot(Clones_glm_df, aes(x = as.factor(binom), y = mismatches, fill = as.factor(binom))) +
    ...
    xlab("Amplified in Hoelzel et al. 1999") +
    ylab("Mismatches at primer binding site [bp]") +
    labs(tag = "A") +
...
## -----------------------------------------------------------------------------
## Load called alleles
load("C:/Users/Jonas/sciebo/MHC/Clones/called_clones-20211027.RData")
...
(plot.B <- ggplot(clone_summary, aes(x,y)) +
    ...
    scale_shape_manual(labels = c("Hoelzel et al., 1999", "0 bp", "1 bp", "2 bp", "3 bp", "4 bp", "5 bp"),
                       values = c(15, 1, 16, 2, 10, 13, 0)) +
    xlab("Sample size") +
    ylab("Number of alleles") +
    scale_x_continuous(breaks = seq(0,60,5)) +
    scale_y_continuous(breaks = seq(0,20,5),
                       limits = c(0,22)) +
    labs(#                                                                      subtitle = "Cloning data",
         tag = "B",
         caption = "Mean and CI estimates based on 999 simulation runs") +
    guides(shape = guide_legend(ncol = 2)))


ggsave(plot.B, filename = "graphics/AlleDetectionFigureNEW.tiff",
       device = "tiff", width = 7.07, height = 9, units = "in", dpi = 400)

(panel <- ggpubr::ggarrange(boxplot.B, plot.B, nrow = 1, ncol = 2, align = "hv"))
ggsave("graphics/sim_panel.png", panel, dpi = 400, width = 16, height = 9, units = "in")
